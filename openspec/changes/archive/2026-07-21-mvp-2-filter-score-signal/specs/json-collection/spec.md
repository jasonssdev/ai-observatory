...
`paper.upvotes` MUST be retained in `raw`, and additionally normalized as
`raw["signal_score"]` (int, equal to `paper.upvotes`) and
`raw["signal_scale"] = "hf_upvotes"`, both additive keys alongside the
original entry.
...
- THEN one `Item` is returned per paper with `url` equal to
  `https://huggingface.co/papers/{paper.id}`, `raw` containing the original
  `upvotes` value, `raw["signal_score"]` equal to `paper.upvotes`, and
  `raw["signal_scale"]` equal to `"hf_upvotes"`
...
The permalink MUST always be retained in `raw`. Each item's `raw` MUST
additionally carry a normalized `raw["signal_score"]` (int, equal to
`hits[].points`) and `raw["signal_scale"] = "hn_points"`, additive keys
alongside the original hit.

#### Scenario: Valid HN payload with external URL
- GIVEN a well-formed HN Algolia hit with a non-null `url`
- WHEN parsed
- THEN the resulting `Item.url` equals the hit's external `url`, and `raw`
  contains `signal_score` equal to the hit's `points` and `signal_scale`
  equal to `"hn_points"`
