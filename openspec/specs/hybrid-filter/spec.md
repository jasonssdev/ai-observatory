...
(e.g. auto-keep priority threshold), each with a documented default applied
when unset, missing, or invalid.

#### Scenario: Defaults apply when unset
- GIVEN no `AIOBS_FILTER_*` environment variables are set
- WHEN configuration is resolved
- THEN each filter setting equals its documented default

#### Scenario: Overrides apply when set
- GIVEN valid `AIOBS_FILTER_*` environment variables are set
- WHEN configuration is resolved
