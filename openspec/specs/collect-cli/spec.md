...
`collect` MUST read `DATA_DIR`, `DB_PATH`, `RECORDS_DIR`, `SOURCES_PATH`, and
`USER_AGENT` from config, honoring `AIOBS_*` env overrides, with hardcoded
defaults when unset.

#### Scenario: Defaults apply when no env vars set
- GIVEN no `AIOBS_*` environment variables are set
- WHEN `collect` runs
- THEN it uses `./data`, `data/observatory.db`, `data/records`, and `./sources.yaml`
